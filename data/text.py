...
Each manuscript can have multiple text pages, each with
a page number, title, and content.
"""
...
def read_one(page_number: str, manuscript_id: str = None) -> dict:
    ...
    if manuscript_id:
        return dbc.read_one(
            TEXT_COLLECT, {
                PAGE_NUMBER: page_number, MANUSCRIPT_ID: manuscript_id})
    else:
...
def exists(page_number: str, manuscript_id: str = None) -> bool:
    ...
    If manuscript_id is provided, check if a text page with
    the given page number exists for that manuscript.
    ...
    if manuscript_id:
        return dbc.read_one(
            TEXT_COLLECT, {
                ...
                MANUSCRIPT_ID: manuscript_id}) is not None
    else:
        return dbc.read_one(TEXT_COLLECT,
                            {PAGE_NUMBER: page_number}) is not None
...
def delete_manuscript_texts(manuscript_id: str):
    ...
    try:
        ...
        print(f"Error deleting manuscript texts: {str(e)}")
        return 0
...
def create(manuscript_id: str, page_number: str, title: str, text: str):
    ...
    if exists(page_number, manuscript_id):
        raise ValueError(
            f'Adding duplicate page_number={page_number} '
            f'for manuscript_id={manuscript_id}')
...
def update(manuscript_id: str, page_number: str, title: str, text: str):
    ...
    try:
        ...
        if is_valid_text(manuscript_id, page_number, title, text):
            ...
            print(
                f"Successfully updated text page {page_number} "
